...
| Detect records that passed a deadline and **act** on them (abandoned carts, payment timeouts, SLA breaches) | Task + directed procedure sweep — **not** TTL |
...
## Anti-Pattern: Using TTL/MIGRATE as a Business Event Generator

A tempting design: give a table a short TTL plus `MIGRATE TO TOPIC`, so that when a row "expires" it lands in Kafka as an event (e.g., a cart with `USING TTL 5 MINUTES` migrating to an `AbandonedEvents` topic). **Do not generate this design.** It works mechanically but misuses the feature:

- **The row is deleted from VoltDB.** The state you usually need for the follow-up action (matching an offer, checking the user profile) is gone, and the event must be re-ingested to act on it — an extra round trip through Kafka for data that never had to leave.
- **The deadline is a column value, not business state.** Extending the timer (user is still shopping) requires an UPDATE to the TTL column on every activity, turning every touch into an extra write whose only purpose is defeating the expiration.
- **False events race with real activity.** Once migration of a row has been triggered, an UPDATE cancels the pending *delete* but **not** the already-started export — the "abandoned" event still fires even though the user came back, and the row also stays in the table (two records eventually migrate).
- **No logic at event time.** Migration exports raw row bytes; there is no hook to run business logic when the deadline passes.

**Generate instead:** keep the state in a normal table with a `last_updated`-style TIMESTAMP column (indexed), and sweep it with a scheduled task running a directed procedure. The procedure selects rows past the deadline, applies the business logic (or emits a proper event via an export stream), and updates or deletes the rows — all transactionally per partition. See the worked example in [ddl-auto-tasks-directed.md](ddl-auto-tasks-directed.md).
