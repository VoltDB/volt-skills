...
Write records to process stdout for observability and debugging.
...
stream.terminateWithSink(
    /* Use Stdout Sink builder/configurator for 'stdout' */
);
```
...
- Pipeline-definition path: `sink.stdout`
- Helm auto-config path: `streaming.pipeline.configuration.sink.stdout`
- Secure values: `--configSecure` or `streaming.pipeline.configurationSecure`

## Helm Notes

- Place sink settings under `streaming.pipeline.configuration.sink.stdout`.
...
- High-volume logging overwhelms I/O and hides business signals.
