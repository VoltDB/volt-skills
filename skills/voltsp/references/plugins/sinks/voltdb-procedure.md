# VoltDB Procedure Sink (Sink)

## Purpose

Call VoltDB procedures transactionally for stateful processing.
...
stream.terminateWithSink(
    /* Use VoltDB Procedure Sink builder/configurator for 'voltdb-procedure' */
);
```
...
- Pipeline-definition path: `sink.voltdb-procedure`
- Helm auto-config path: `streaming.pipeline.configuration.sink.voltdb-procedure`
- Secure values: `--configSecure` or `streaming.pipeline.configurationSecure`

## Helm Notes

- Place sink settings under `streaming.pipeline.configuration.sink.voltdb-procedure`.
...
- Procedure name/signature or parameter mapping mismatch causes runtime failures.
